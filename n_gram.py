...
class NGramModel:  # ngram model class
    ...
    def train(self, data):  # train n gram model
        for sentence in data:  # loop through sentences in training set
            words = self.tokenize_and_preprocess(sentence)  # tokenize sentence
    ...
    def smoothing_laplace_probability(self, prefix, word):  # find laplace smoothed probability
    ...
    def smoothing_addk_probability(self, prefix, word, k=0.5):  # compute add k smoothed probability
        if self.n == 1:  # check if unigram 
    ...
    def get_the_perplexity(self, data): 
        ...
        for sentence in data:  # loop through validation sentences
            ...
            for i in range(len(words) - self.n + 1):  # loop through n grams
                ...
                prob = self.get_the_probabilities(prefix, word)  # compute %
    ...
    def show_model_probabilities(self):  # display model probabilities for unigram and bigram
    ...
    def get_the_probabilities(self, prefix, word):  # find probability with/without smoothing
        if self.smoothing == 'laplace':  # if laplace smoothing 
            return self.smoothing_laplace_probability(prefix, word)  # return laplace probability
        elif self.smoothing == 'add-k':  # if add k smoothing
            return self.smoothing_addk_probability(prefix, word, self.alpha)  # return add k probability
...
if __name__ == "__main__":  # main 
    ...
    unigram.show_model_probabilities()  # display unigram %
    
    bigram = NGramModel(n=2)  # bigram model instance
    bigram.train(train_data)  # train bigram model
    bigram.show_model_probabilities()  # display bigram %
    print("validation: unigram perplexity:", unigram.get_the_perplexity(val_data))  # find unigram perplexity
    print("validation: bigram perplexity:", bigram.get_the_perplexity(val_data))  # find bigram perplexity
    ...
    print("validation: laplace unigram perplexity:", laplace_unigram.get_the_perplexity(val_data))  # find perplexity
    ...
    print("validation: add-k bigram perplexity:", add_k_bigram.get_the_perplexity(val_data))  # compute perplexity

    print("training: unigram perplexity:", unigram.get_the_perplexity(train_data))  
    print("training: bigram perplexity:", bigram.get_the_perplexity(train_data))  
    print("training: laplace unigram perplexity:", laplace_unigram.get_the_perplexity(train_data))  
    print("training: add-k bigram perplexity:", add_k_bigram.get_the_perplexity(train_data))  
